...
do
	if [ -f downloads/$ff ]; then
		cabextract --lowercase -F '*.ttf' --directory=cab-contents downloads/$ff
	fi
	cp --preserve=timestamps cab-contents/*.ttf fonts
	rm -f cab-contents/*
done

cabextract --lowercase --directory=cab-contents downloads/wd97vwr32.exe
cabextract --lowercase -F '*.ttf' --directory=cab-contents cab-contents/viewer1.cab
cp --preserve=timestamps cab-contents/*.ttf fonts/
rm -f cab-contents/*

cabextract --lowercase -F '*.ttf' --directory=cab-contents downloads/EUupdate.EXE
cp --preserve=timestamps cab-contents/*.ttf fonts/
...
cp *.ttf fonts.scale fonts.dir $RPM_BUILD_ROOT/%{fontdir}/
