...
font payload is included in the RPM, so installation does not need network
access to the SourceForge mirrors.
...
2. calculates the RPM release from the number of commits reachable from
   `HEAD`;
3. builds the RPM and SRPM in a Fedora 43 container; and
4. exports both artifacts to `rpms/`.
...
The fonts are tracked directly by the RPM and are removed with the package.
...
3. Review the source tree under `sources/`, the spec, `builder`, and `Makefile`.
